...
Engine::Engine() {
    ...
    setupText(&titleText, mainFont, "Snake Game", 28, Color::Blue);
    ...
    setupText(&StickmanEatenText, mainFont, "StickMan 0", 28, Color::Blue);
...
void Engine::startTheGame() {
    ...
    StickmanEatenThisLevel = 0;
    StickmanEatenTotal = 0;
    currentLevel = 1;
...
void Engine::moveStickman() {
    // Find a location to place the Stickman
    // Must not be inside the snake, or on a wall

    // Divide the field into sections the size of the Stickman - remove 2 to exclude the exterior walls
    Vector2f StickmanResolution = Vector2f(resolution.x / 20 - 2, resolution.y / 20 - 2);
    Vector2f newStickmanLocation;
    ...
    do {
        ...
        newStickmanLocation.x = (float)(1 + rand() / ((RAND_MAX + 1u) / (int)StickmanResolution.x)) * 20;
        newStickmanLocation.y = (float)(1 + rand() / ((RAND_MAX + 1u) / (int)StickmanResolution.y)) * 20;

        // Check if it is in the snake
        for (auto& s : snake) {
            if (s.getShape().getGlobalBounds().intersects(
                Rect<float>(newStickmanLocation.x, newStickmanLocation.y, 20, 20))) {
                badLocation = true;
                break;
            }
        }
    ...
    stickman.setPosition(newStickmanLocation);
}
